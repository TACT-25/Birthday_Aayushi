...
const stitchImg = new Image(); stitchImg.src = "Basket.png";
...
let cakes = [], score = 0, missed = 0, timeLeft = 30, highScore = 0;
...
function checkCollision() {
  for (let i = cakes.length - 1; i >= 0; i--) {
    const cake = cakes[i];
    if (cake.x > stitch.x && cake.x < stitch.x + stitch.width &&
        cake.y + 25 > stitch.y && cake.y - 25 < stitch.y + stitch.height) {
      switch (cake.type) {
        ...
        case "timer": timeLeft += 10; soundBonus.play(); break;
...
function spawnCake() {
  ...
  let type = "normal", img = cupcakeImgs[Math.floor(Math.random() * cupcakeImgs.length)];
...
function gameLoop() {
  ...
  drawStitch(); drawCakes(); updateCakes(); checkCollision();
...
function endGame(bombCaught = false) {
  ...
  const message = bombCaught ? "💥 You caught a cherry bomb! Game Over!" :
...
document.addEventListener("DOMContentLoaded", () => {
  document.getElementById("leftBtn").addEventListener("click", () => {
    stitch.x -= 30; if (stitch.x < 0) stitch.x = 0;
  });
  document.getElementById("rightBtn").addEventListener("click", () => {
    stitch.x += 30; if (stitch.x + stitch.width > canvas.width) stitch.x = canvas.width - stitch.width;
  });
...
// Preload images before enabling the game
